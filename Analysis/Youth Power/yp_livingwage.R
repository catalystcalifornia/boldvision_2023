...
ppl$wks_worked <- as.numeric(ifelse(!is.na(ppl$wkwn), ppl$wkwn, ppl$wks_worked_avg))
...
## combine race/eth estimates with total
lw_county <-  
  bind_rows(lw_race_county %>% filter(race != "white" & race != "asian" & race != "black" & 
...
lw_county_id <-
  ...
  select(county, race, count, pop, rate, rate_cv, count_cv, asbest, values_count, best, diff, index_of_disparity) %>%
  filter(race!= 'white' & race!= 'black'& race!= 'asian'& race!= 'other'& race!= 'twoormor'& race!= 'nh_aian'& race!= 'nh_pacisl'& race!= 'nh_swana')
...
COMMENT ON COLUMN bv_2023.yp_livingwage_subgroup.count_cv is 'CV (coefficient of variation) for count of youth who areare/are not making above living wage (see column var for which category). Values over .4, i.e. 40%, are generally considered to be unstable estimates.';
...
lw_spa_id <-
  ...
  mutate(index_of_disparity = ((sum(diff, na.rm=T)/best/(values_count - 1))*100)) %>% # use the best value identified above
  #select just the columns needed 
  select(spa_id, spa_name, count, pop, rate, rate_cv, count_cv, asbest, values_count, best, diff, index_of_disparity)
...
COMMENT ON COLUMN bv_2023.yp_livingwage_region.count_cv is 'CV (coefficient of variation) for count of youth who are/are not earning above living wage  (see column var for which category). Values over .4, i.e. 40%, are generally considered to be unstable estimates.';
