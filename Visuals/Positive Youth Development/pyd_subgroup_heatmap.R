#Create Positive Youth Development Heatmap, BIPOC chart, and Index of Disparity chart
...
df_no_total <-pyd_domain %>% filter(subgroup!="Total" & subgroup!="BIPOC")
...
heatmap <- ggplot(df_no_total, aes(x=subgroup, y=reorder(category,index_of_disparity), fill= avg_rank, height=0.6, width=0.6)) + #height and width change the size of the squares
  ...
  labs(title = "<b>Mapping <i>racial disparity</i> across positive youth development systems in LA County</b>", 
      ...
      or North African/Middle Eastern or North African</span> <br>Asthma rates for AIAN and NHPI youth are unstable. See boldvisionla.org for definitions and methodology.")+  
...
bipoc_chart <- ggplot(df_bipoc, aes(x=subgroup, y=reorder(category,index_of_disparity), fill= avg_rank, height=0.3, width=0.3)) + 
  ...
  labs(title = "**BIPOC youth outcomes<br> across positive youth development<br> systems in LA County**", 
       caption = "<br>BIPOC rates represent how youth of color are doing on average<br>across each indicator,
       e.g., at a rate of 33%, youth of color have access <br>to early childhood education.<br>BIPOC=Black, Indigenous, People of Color.<br>See boldvisionla.org for definitions and methodology.")+ 
...
ggsave(plot=bipoc_chart,
       file="W:/Project/OSI/Bold Vision/BV 2023/Deliverables/Positive Youth Development/pyd_bipoc_r.pdf",
...
id_barchart <- ggplot(id_data, aes(x=reorder(category,index_of_disparity), y=index_of_disparity, height=0.6, width=0.6)) +
  ...
  labs(title="<b>Youth face the highest <i>racial disparity</i> in asthma rates</b>", 
