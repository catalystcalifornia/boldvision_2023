#Create Systems Impact Heatmap and Index of Disparity chart for regions
...
# heatmap ----
# edit probation/foster label for regions

si_domain$category<-ifelse(grepl("Youth in Foster Care / Probation Systems",si_domain$category),"Youth in Foster Care Systems", si_domain$category)

#remove total and bipoc columns
df_no_total <-si_domain 
...
heatmap <- ggplot(df_no_total, aes(x=spa, y=reorder(category,index_of_disparity), fill= avg_rank, height=0.6, width=0.5)) + #height and width change the size of the squares #originally height=0.6, width = 0.6
  ...
  scale_fill_gradientn(colors = c(darkest_blue,blue,dim_blue, dimmer_blue, dimmest_blue, lighest_pink, lighter_pink, light_pink, dark_pink, darker_pink), na.value=light_gray, 
                       name="", labels=c('', '', '', '', '')) + 
  labs(title = "<b>Mapping <i>regional disparities</i> across systems impact indicators in LA County</b>", 
...
ggsave(plot=heatmap,
       file="W:/Project/OSI/Bold Vision/BV 2023/Deliverables/Systems Impact/R Draft Files/si_region_heatmap_r.pdf",
...
id_barchart <- ggplot(id_data, aes(x=reorder(category,index_of_disparity), y=index_of_disparity, height=0.6, width=0.6)) +
  ...
  labs(title="<b>Youth face the highest <i>regional disparities</i> in documentation <br>status</b>", 
  ...
  geom_text(aes(label = paste0(index_of_disparity,"%")), family = font_bar_label, hjust = 1.5, size = 7, color="White") +   # format data labels, adjust hjust to avoid overlap w/ total line # , hjust = 1.7
...
id_barchart <- id_barchart + theme(
  ...
  axis.text.y = element_text(size = 19, family= font_axis_label, lineheight = 1, hjust=0),
  axis.ticks = element_blank(),
...
ggsave(plot=id_barchart,
       file="W:/Project/OSI/Bold Vision/BV 2023/Deliverables/Systems Impact/R Draft Files/si_region_id_barchart_r.pdf",
