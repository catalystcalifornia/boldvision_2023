#Create Systems Impact Heatmap, BIPOC chart, and Index of Disparity chart
...
df_no_total <-si_domain %>% filter(subgroup!="Total" & subgroup!="BIPOC")
...
heatmap <- ggplot(df_no_total, aes(x=subgroup, y=reorder(category,index_of_disparity), fill= avg_rank, height=0.6, width=0.6)) + #height and width change the size of the squares
  ...
  labs(title = "<b>Mapping <i>racial disparity</i> across systems impact indicators in LA County</b>", 
       ...
       <span style='font-size:14pt'>Race Note: AIAN=American Indian and Alaskan Native; NHPI=Native Hawaiian or Pacific Islander; 
       <br> SWANA/SA=Southwest Asian (Middle Eastern) or North African/South Asian; API = Asian Pacific Islander</span>
...
ggsave(plot=heatmap,
       file="W:/Project/OSI/Bold Vision/BV 2023/Deliverables/Systems Impact/R Draft Files/si_subgroup_heatmap_r.pdf",
       units = c("in"),  width = 18, height=10, device = cairo_pdf)

# BIPOC chart----
df_bipoc <- si_domain %>% filter(subgroup=="BIPOC")
...
bipoc_chart <- ggplot(df_bipoc, aes(x=subgroup, y=reorder(category,index_of_disparity), fill= avg_rank, height=0.3, width=0.3)) + 
  ...
  labs(title = "**BIPOC youth outcomes<br> across systems impact indicators<br>in LA County**", 
       caption = "<br>BIPOC rates represent how youth of color are doing on average<br>across each indicator,
       e.g. at a rate of 28.7%, youth of color are <br>undocumented or live with undocumented family.<br>BIPOC=Black, Indigenous, People of Color.
       <br>See boldvisionla.org for definitions and methodology.")+ 
...
bipoc_chart <- bipoc_chart + theme(
  ...
  axis.text.y = element_text(size = 14, family= font_axis_label, lineheight = 1, hjust=0),
  axis.ticks = element_blank(),
...
ggsave(plot=bipoc_chart,
       file="W:/Project/OSI/Bold Vision/BV 2023/Deliverables/Systems Impact/R Draft Files/si_bipoc_r.pdf",
       units = c("in"), width = 6, height=6, device = cairo_pdf)

#index of disparity chart ----

id_data <- si_domain %>% select(category, index_of_disparity) %>% distinct() %>% filter(!is.na(index_of_disparity))
...
id_barchart <- ggplot(id_data, aes(x=reorder(category,index_of_disparity), y=index_of_disparity, height=0.6, width=0.6)) +
  ...
  labs(title="<b>Youth face the highest <i>racial disparity</i> in youth arrests </b>", 
  ...
  geom_text(aes(label = paste0(round((index_of_disparity), 1),"%")), family = font_bar_label, hjust = 1.7, size = 7, color="White") +   # format data labels, adjust hjust to avoid overlap w/ total line
  scale_x_discrete(labels = function(category) str_wrap(category, width = 19)) +            # wrap long labels
  theme_void()+
  xlab("") +
  ylab("") +
  coord_flip()
...
ggsave(plot=id_barchart,
       file="W:/Project/OSI/Bold Vision/BV 2023/Deliverables/Systems Impact/R Draft Files/si_subgroup_id_barchart_r.pdf",
