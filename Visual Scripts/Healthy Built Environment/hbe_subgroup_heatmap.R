# Create Healthy Built Environment Heatmap, BIPOC chart, and Index of Disparity chart for subgroups/racial groups
...
df_no_total <-hbe_domain %>% filter(subgroup!="Total" & subgroup!="BIPOC")
...
heatmap <- ggplot(df_no_total, aes(x=subgroup, y=reorder(category,index_of_disparity), fill= avg_rank, height=0.6, width=0.6)) + #height and width change the size of the squares
  ...
  labs(title = "<b>Mapping <i>racial disparity</i> across healthy built environments in LA County</b>", 
      ...
      or North African/Middle Eastern or North African</span> <br>See boldvisionla.org for definitions and methodology.")+  
...
# ggsave(plot=heatmap,
#        file="W:/Project/OSI/Bold Vision/BV 2023/Deliverables/Healthy Built Environment/hbe_subgroup_heatmap_r.pdf",
#        units = c("in"),  width = 18, height=10, device = cairo_pdf)

# BIPOC chart----
df_bipoc <- hbe_domain %>% filter(subgroup=="BIPOC")
...
bipoc_chart <- ggplot(df_bipoc, aes(x=subgroup, y=reorder(category,index_of_disparity), fill= avg_rank, height=0.3, width=0.3)) + 
  ...
  labs(title = "**BIPOC youth outcomes<br> across healthy built environments<br>in LA County**", 
       caption = "<br>BIPOC rates represent how youth of color are doing on average<br>across each indicator,
       e.g. 37.2 BIPOC youth per 100k are severely <br>injured or die from traffic collisions.<br>%ile=Average Percentile. BIPOC=Black, Indigenous, People of Color. <br>See boldvisionla.org for definitions and methodology.")+ 
...
#        file="W:/Project/OSI/Bold Vision/BV 2023/Deliverables/Healthy Built Environment/hbe_bipoc_r.pdf",
#        units = c("in"), width = 6, height=6, device = cairo_pdf)

#index of disparity chart ----

id_data <- hbe_domain %>% select(category, index_of_disparity) %>% distinct() %>% filter(!is.na(index_of_disparity))
...
id_barchart <- ggplot(id_data, aes(x=reorder(category,index_of_disparity), y=index_of_disparity, height=0.6, width=0.6)) +
  ...
  labs(title="<b>Youth face the highest <i>racial disparity</i> <br>in traffic injuries and fatalities </b>", 
       subtitle = "Average racial disparity between groups by indicator",
       caption="<br>The length of the bar shows the average disparity between groups for each indicator. <br>The longer the bar, the greater the disparity. <br>See boldvisionla.org for definitions and methodology.") + 
...
#        file="W:/Project/OSI/Bold Vision/BV 2023/Deliverables/Healthy Built Environment/hbe_subgroup_id_barchart_r.pdf",
