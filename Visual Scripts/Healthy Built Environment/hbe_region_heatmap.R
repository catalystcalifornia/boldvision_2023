# Create Healthy Built Environment Heatmap and Index of Disparity charts for regions
...
df_no_total <-hbe_domain 

heatmap <- ggplot(df_no_total, aes(x=spa, y=reorder(category,index_of_disparity), fill= avg_rank, height=0.6, width=0.5)) + #height and width change the size of the squares #originally height=0.6, width = 0.6
  ...
  labs(title = "<b>Mapping <i>regional disparities</i> across healthy built environments in LA County</b>", 
...
# ggsave(plot=heatmap,
#        file="W:/Project/OSI/Bold Vision/BV 2023/Deliverables/Healthy Built Environment/R Draft Files/hbe_region_heatmap_r.pdf",
#        units = c("in"),  width = 18, height=10, device = cairo_pdf)

# add pop-up and modify legend in illustrator

#index of disparity chart ----

id_data <- hbe_domain %>% distinct(category, index_of_disparity) %>%filter(!is.na(index_of_disparity))

id_barchart <- ggplot(id_data, aes(x=reorder(category,index_of_disparity), y=index_of_disparity, height=0.7, width=0.7)) + #, height=0.6, width=0.6
  geom_chicklet(radius = grid:: unit(3, "mm"), fill=blue) + #rounded corners bar chart
  labs(title="<b>Youth face the highest <i>regional disparities</i> in pollution exposure", 
       subtitle = "Average disparity between regions by indicator",
       caption="The length of the bar shows the average disparity between regions for each indicator. 
       <br>The longer the bar, the greater the disparity.
       <br>See boldvisionla.org for definitions and methodology.") + 
  geom_text(aes(label = paste0(index_of_disparity,"%")), family = font_bar_label, hjust = 1.8, size = 7, color="White") +   # format data labels, adjust hjust to avoid overlap w/ total line #, hjust = 1.7,  size = 7,
...
#        file="W:/Project/OSI/Bold Vision/BV 2023/Deliverables/Healthy Built Environment/R Draft Files/hbe_region_id_barchart_r.pdf",
#        units = c("in"),  width = 12, height=8, device = cairo_pdf) #, width = 12

# add pop-up, adjust short bar labels, and adjust some indicator labels in illustrator
