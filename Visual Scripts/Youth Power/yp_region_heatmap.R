# Create Youth Power Heatmap and Index of Disparity chart for regions
...
#remove extra variables that are not SPA level
df_no_total <-yp_domain%>%filter(!category %in% c("Voter Turnout","Political Engagement and Advocacy"))
...
heatmap <- ggplot(df_no_total, aes(x=spa, y=reorder(category,index_of_disparity), fill= avg_rank, height=0.6, width=0.5)) + #height and width change the size of the squares #originally height=0.6, width = 0.6
  ...
  scale_fill_gradientn(colors = c(darkest_blue,blue,dim_blue, dimmer_blue, dimmest_blue, lighest_pink, lighter_pink, light_pink, dark_pink, darker_pink), na.value=light_gray,
                       name="", labels=c('', '', '', '', '')) +
  labs(title = "<b>Mapping <i>regional disparities</i> across youth power systems in LA County</b>",
...
# ggsave(plot=heatmap,
#        file="W:/Project/OSI/Bold Vision/BV 2023/Deliverables/Youth Power/R Draft Files/yp_region_heatmap_r.pdf",
#        units = c("in"),  width = 18, height=8, device = cairo_pdf)#originally width = 18, height=10


#index of disparity chart ----

id_data <- yp_domain %>% select(category, index_of_disparity) %>% distinct() %>% filter(!is.na(index_of_disparity))%>%filter(!category %in% c("Voter Turnout","Political Engagement and Advocacy"))
...
id_barchart <- ggplot(id_data, aes(x=reorder(category,index_of_disparity), y=index_of_disparity, height=0.6, width=0.6)) +
  ...
  labs(title="<b>Youth face the highest <i>regional disparities</i> in access to base-building <br>organizations</b>", 
...
#        file="W:/Project/OSI/Bold Vision/BV 2023/Deliverables/Youth Power/R Draft Files/yp_region_id_barchart_r.pdf",
