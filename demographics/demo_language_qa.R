...
# QA
# Load the people PUMS data
people_qa <- fread(paste0(root, "CA_2021/psam_p06.csv"), header = TRUE, data.table = FALSE,
                colClasses = list(character = c("PUMA", "LANX"))) # don't bring in age as a character


# Load the housing PUMS data
housing_qa <- fread(paste0(root, "CA_2021/psam_h06.csv"), header = TRUE, data.table = FALSE, colClasses = list(character = c("PUMA", "HHL")))

# revise function to tally over a single column with values we need
pums_run_qa <- function(x){
  weight <- 'PWGTP' # using WGTP b/c calculating percentage of rent-burdened households
  repwlist = rep(paste0("PWGTP", 1:80))
...
# adjust filtering for what we need
eligible_hhs_qa <- people_qa %>% 
  
  #filtering for universe and LA county
  filter(grepl('037', PUMA) & AGEP < 25)   %>% 
  
  # join their housing info  
  left_join(housing_qa %>% filter(grepl('037', PUMA)), 
            by = c("SERIALNO", "PUMA")) %>%
...
# add geoid and indicator
eligible_hhs_qa$geoid <- "037"

# recode the values for language at home
eligible_hhs_qa$indicator=(ifelse(eligible_hhs_qa$HHL == 1, "English", 
                                  ifelse(eligible_hhs_qa$HHL == 2, "Spanish",
                                  ifelse(eligible_hhs_qa$HHL == 3, "Other Indo-European language",
                                  ifelse(eligible_hhs_qa$HHL == 4, "Asian and Pacific Island languages",
                                  ifelse(eligible_hhs_qa$HHL == 5, "Other Language", "Other"))))))
# check that we were left with no 'Other' values that weren't recoded
table(eligible_hhs_qa$indicator)

# run new function
df <- pums_run_qa(eligible_hhs_qa)
# looks good, so we can say about 73% of youth 0-24 in LA County speak a language other than English at home
