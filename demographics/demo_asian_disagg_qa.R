...
eligible_hhs_qa <- people %>% 
  
  #filtering for universe and LA county
  filter(
    # !is.na(RACASN) & # no NAs to filter for
    grepl('037', PUMA) & 
      AGEP < 25 & RACASN == 1 
    # & !is.na(RAC2P) # no need to filter for NA
  )   
# %>% 
#   
#remove records with no weights
# filter(!is.na(PWGTP)) %>% # no need to filter for NA

#filter for age 0-5 and select distinct households
# distinct(SERIALNO, .keep_all = TRUE) # no need to filter for distinct households--we are analyzing people not HHLDS


weight <- 'PWGTP'  # weight
repwlist = rep(paste0("PWGTP", 1:80)) # replicate weights

asian_youth<-eligible_hhs%>%rename(asian_subgroup=Description) # clarify column we'll group by and set final dataset
table(asian_youth$AGEP) # check that ages look good
table(asian_youth$RAC1P) # includes Asian Alone and Two or More RAces, maybe just filter for Asian alone since Two or More will be addressed in Multiracial cut

# set up survey design of final dataset
asian_youth_svry <- asian_youth %>%               
  as_survey_rep(
...
###### Asian subgroups, including alone and in combo ######
asian_subgroups_table <- asian_youth_svry %>% # the data and survey design
...
# make everything under 1% Another Asian Identity Alone
# exclude two or more races

# test
asian_alone_youth <- left_join(eligible_hhs_qa, race_codes, by=("RAC2P"))%>%filter(RAC1P==6)%>%rename(asian_subgroup=Description)%>%mutate(geoid='037')
