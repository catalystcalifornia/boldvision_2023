...
# QA
eligible_hhs<-eligible_hhs%>%mutate(latine=ifelse(HISP==1,"No","Yes"))
eligible_hhs_test_latine <- eligible_hhs %>% group_by(Description, latine,geoid) %>% summarize(indicator = n())

# filter out latine for now to focus on nh multiracial youth
nh_multiracial_youth<-eligible_hhs%>%filter(latine=="No")
...
multiracial_youth_svry <- nh_multiracial_youth %>%               
  as_survey_rep(
    variables = c(geoid, Description),   # dplyr::select grouping variables
...
multiracial_youth_table <- multiracial_youth_svry %>%
  group_by(geoid,Description) %>%   # group by asian subgroup description
  summarise(
    num = survey_total(na.rm=T), # get the (survey weighted) count for the numerators
    rate = survey_mean()) %>%        # get the (survey weighted) proportion for the numerator
